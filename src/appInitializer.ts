...
export function createWindow(splash?, time?) {
  startTime = time;
  logStartupTime("createWindowMethod called");
  //initialize the environment variables
  initializeEnv();
  let telemetryInstance = containerAPI.getTelemetrySDKInstance().getInstance();
  ...
  win.webContents.once("dom-ready", () => {
    ...
    logStartupTime("Main browser window loaded");
    win.show();
    win.maximize();
  });
  ...
  bootstrapDependencies()
    .then(() => {
      logStartupTime("App bootstrapped");
...
const initializeEnv = () => {
  ...
  if (app.isPackaged) {
    envs = JSON.parse(
      new Buffer("ENV_STRING_TO_REPLACE", "base64").toString("ascii")
    ); // deployment step will replace the base64 string
...
const bootstrapDependencies = async () => {
  ...
  logStartupTime("copyPluginsMetaData");
  await setAvailablePort();
  logStartupTime("setAvailablePort");
  await Promise.all([framework(), checkPluginsInitialized()]);
  logStartupTime("bootstrapFramework");
  await containerAPI.bootstrap();
  logStartupTime("openRapInitialized");
  await startApp();
  logStartupTime("startApp");
...
const makeImportApiCall = async (contents: Array<string>) => {
  if (_.isEmpty(contents)) {
    logger.error(
      "Content import api call error",
      "Reason: makeImportApiCall called with empty array"
    );
    return;
  }
